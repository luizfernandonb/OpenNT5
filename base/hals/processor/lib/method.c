...
--*/
{
    ...
    *AcpiMulti = (PACPI_BIOS_MULTI_NODE) ExAllocatePoolWithTag(NonPagedPool,
	...
	// openxp: ULONG replaced with ULONG_PTR, as the ULONG type is for x86-only architecture, and we're making cross-platform operating-system :D
    //
    //try {

        int15Info = (PLEGACY_GEYSERVILLE_INT15)&(multiNode->E820Entry[multiNode->Count]);
        if (((ULONG_PTR)int15Info - (ULONG_PTR)valueInfo) <= valueInfo->DataLength && 
