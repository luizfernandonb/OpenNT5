...
REM set CPFile=%_NTPOSTBLD%\build_logs\CPLocation.txt
REM if not exist %CPFile% (
REM    call logmsg.cmd "Copy Location file not found, will attempt to create ..."
REM    call %RazzleToolPath%\PostBuildScripts\CPLocation.cmd -l:%lang%
REM    if not exist %CPFile% (
REM       call errmsg.cmd "CPLocation.cmd failed, exiting ..."
REM       goto :End
REM    )
REM )
REM for /f "delims=" %%a in ('type %CPFile%') do set CPLocation=%%a
REM if not exist %CPLocation% (
REM    call logmsg.cmd "Copy Location from %CPFile% does not exist, retry ..."
REM    call %RazzleToolPath%\PostBuildScripts\CPLocation.cmd -l:%lang%
REM    if not exist %CPFile% (
REM       call errmsg.cmd "CPLocation.cmd failed, exiting ..."
REM       goto :End
...
REM    for /f "delims=" %%a in ('type %CPFile%') do set CPLocation=%%a
REM    if not exist !CPLocation! (
REM       call errmsg.cmd "Copy Location !CPLocation! does not exist ..."
REM       goto :End
REM    )
REM )

set CPLocation=%_NTDRIVE%\binaries.x86%_BUILDTYPE%
...
REM set CPFile=%_NTPOSTBLD%\build_logs\CPLocation.txt
REM if not exist %CPFile% (
REM    call errmsg.cmd "Failed to find %CPFile%, exiting ..."
REM    goto :End
REM )
REM for /f "delims=" %%a in ('type %CPFile%') do set CPLocation=%%a
REM if not exist %CPLocation% (
REM    call errmsg.cmd "Copy location %CPLocation% not found, exiting ..."
REM    goto :End
REM )
