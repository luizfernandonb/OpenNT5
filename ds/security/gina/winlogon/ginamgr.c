...
BOOL
WINAPI
DummyWlxGetConsoleSwitchCredentials(
    PVOID Context,
    PVOID pCredInfo
    )
{
...
VOID
WINAPI
DummyWlxReconnectNotify(PVOID pContext)
{
}

VOID
WINAPI
DummyWlxDisconnectNotify(PVOID pContext)
{
}
    ...
    PBOOL       isBadGina)
{
    ...
    *isBadGina = 0;
    ...
    //
    // New interfaces for NT 5.1
    //

    pTerm->Gina.pWlxGetConsoleSwitchCredentials = (PWLX_GETCONSOLESWITCHCREDENTIALS) GetProcAddress( pTerm->Gina.hInstance, "WlxGetConsoleSwitchCredentials" );
    if ( !pTerm->Gina.pWlxGetConsoleSwitchCredentials )
    {
        pTerm->Gina.pWlxGetConsoleSwitchCredentials = DummyWlxGetConsoleSwitchCredentials ;
    }

    pTerm->Gina.pWlxReconnectNotify = (PWLX_RECONNECTNOTIFY) GetProcAddress( pTerm->Gina.hInstance, "WlxReconnectNotify" );
    if ( !pTerm->Gina.pWlxReconnectNotify )
    {
        pTerm->Gina.pWlxReconnectNotify = DummyWlxReconnectNotify ;
    }

    pTerm->Gina.pWlxDisconnectNotify = (PWLX_DISCONNECTNOTIFY) GetProcAddress( pTerm->Gina.hInstance, "WlxDisconnectNotify" );
    if ( !pTerm->Gina.pWlxDisconnectNotify )
    {
        pTerm->Gina.pWlxDisconnectNotify = DummyWlxDisconnectNotify ;
    }
    ...
    if (!g_Console && dwGinaLevel == WLX_CURRENT_VERSION && pTerm->Gina.pWlxGetConsoleSwitchCredentials == DummyWlxGetConsoleSwitchCredentials)
    {
        DebugLog((DEB_ERROR, "%ws claims to be at version 0x%lx, but it is missing API entry %ws, can't support this GINA for remote desktops\n", lpGinaName, dwGinaLevel, L"WlxGetConsoleSwitchCredentials"));
        goto LoadGina_ErrorReturn;
    }
    ...
    HWND    hDlg)
{
    ...
    WCHAR   Format[ MAX_PATH * 2 ];
    WCHAR   Dll[ MAX_PATH ];

    if (g_Console) {
        LoadString( GetModuleHandle(NULL), IDS_BAD_GINA, Format, MAX_PATH );
    } else {
        LoadString( GetModuleHandle(NULL), IDS_NDDE_INCOMPATIBLE, Format, MAX_PATH );
    }
    ...
    )
{
        WlxDialogBox( BadGinahWlx,
                      ...
                      MAKEINTRESOURCE( IsActiveConsoleSession() ? IDD_BAD_GINA_DIALOG : IDD_BAD_REMOTE_GINA_DIALOG ),
                      NULL,
                      BadGinaDlgProc );
...
VOID
WINAPI
BadGinaReconnectNotify(PVOID pContext)
{
}

VOID
WINAPI
BadGinaDisconnectNotify(PVOID pContext)
{
}
    ...
    PTERMINAL pTerm )
{
    ...
    pTerm->Gina.pWlxReconnectNotify = BadGinaReconnectNotify;
    pTerm->Gina.pWlxDisconnectNotify = BadGinaDisconnectNotify;
}
