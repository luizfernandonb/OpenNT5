...
extern int IsMachinePolicyFgRefreshSync(PTERMINAL arg_0);
...
char * StateNames[] = {"Preload", "Initialize", "NoOne", "NoOne_Display",
                        ...
                        "WaitForLogoff", "WaitForShutdown", "PostSetupShutdown", "Shutdown" };
#endif
    ...
    DWORD       SasType)
{
    ...
    if (pTerm->cActiveWindow)
    {
        ...
        if ( ( SasType > WLX_SAS_TYPE_MAX_MSFT_VALUE) ||
             ...
             ( SasType == WLX_SAS_TYPE_AUTHENTICATED ) )
        {
            //
    ...
    LPARAM  lParam)
{
    ...
    MapTerminal.pMap = pMap;
    MapTerminal.pTerm = pTerm;
    res = (int)Fusion_DialogBoxIndirectParam(hInstance, lpTemplate, hWnd, RootDlgProc, (LPARAM) &MapTerminal);
    ...
    )
{
    ...
    switch ( Option )
    {
        ...
        case WLX_OPTION_SMART_CARD_INFO:
            ...
            if (Value) {
                DWORD dwCardSize = 0;
                DWORD dwReaderSize = 0;
                DWORD dwContainerSize = 0;
                DWORD dwCryptoProviderSize = 0;
                PCRITICAL_SECTION CriticalSection = &pTerm->CurrentScCritSect ;
                PSC_DATA ScData;
                PWLX_SC_NOTIFICATION_INFO pScInfo;
                EnterCriticalSection( CriticalSection );
                ScData = pTerm->CurrentScData;
                if ( ScData != NULL ) {
                    if ( ScData->ScInfo.pszCard != NULL ) {
                        dwCardSize = wcslen( ScData->ScInfo.pszCard ) + 1;
                    }
                    if ( ScData->ScInfo.pszReader != NULL ) {
                        dwReaderSize = wcslen( ScData->ScInfo.pszReader ) + 1;
                    }
                    if ( ScData->ScInfo.pszContainer != NULL ) {
                        dwContainerSize = wcslen( ScData->ScInfo.pszContainer ) + 1;
                    }
                    if ( ScData->ScInfo.pszCryptoProvider != NULL ) {
                        dwCryptoProviderSize = wcslen( ScData->ScInfo.pszCryptoProvider ) + 1;
                    }
                    pScInfo = LocalAlloc( LMEM_FIXED | LMEM_ZEROINIT, sizeof(*pScInfo) + (dwCardSize + dwReaderSize + dwContainerSize + dwCryptoProviderSize) * sizeof(WCHAR) );
                    if ( pScInfo != NULL ) {
                        if ( dwCardSize != 0 ) {
                            pScInfo->pszCard = (LPWSTR)((LPBYTE)pScInfo + sizeof(*pScInfo));
                            memcpy( pScInfo->pszCard, pTerm->CurrentScData->ScInfo.pszCard, dwCardSize * sizeof(WCHAR) );
                        }
                        if ( dwReaderSize != 0 ) {
                            pScInfo->pszReader = (LPWSTR)((LPBYTE)pScInfo + sizeof(*pScInfo)) + dwCardSize;
                            memcpy( pScInfo->pszReader, pTerm->CurrentScData->ScInfo.pszReader, dwReaderSize * sizeof(WCHAR) );
                        }
                        if ( dwContainerSize != 0 ) {
                            pScInfo->pszContainer = (LPWSTR)((LPBYTE)pScInfo + sizeof(*pScInfo)) + dwCardSize + dwReaderSize;
                            memcpy( pScInfo->pszContainer, pTerm->CurrentScData->ScInfo.pszContainer, dwContainerSize * sizeof(WCHAR) );
                        }
                        if ( dwCryptoProviderSize != 0 ) {
                            pScInfo->pszCryptoProvider = (LPWSTR)((LPBYTE)pScInfo + sizeof(*pScInfo)) + dwCardSize + dwReaderSize + dwContainerSize;
                            memcpy( pScInfo->pszCryptoProvider, pTerm->CurrentScData->ScInfo.pszCryptoProvider, dwCryptoProviderSize * sizeof(WCHAR) );
                        }
                    }
                    *Value = (ULONG_PTR) pScInfo ;
                } else {
                    *Value = (ULONG_PTR) NULL ;
                }
...
{
    WLX_CLIENT_CREDENTIALS_INFO_V2_0 Credentials;
    BOOL ret;

    ret = WlxQueryTsLogonCredentials( &Credentials );

    if ( ret ) {
        pInfo->pszUserName = Credentials.pszUserName;
        pInfo->pszDomain = Credentials.pszDomain;
        pInfo->pszPassword = Credentials.pszPassword;
        pInfo->fPromptForPassword = Credentials.fPromptForPassword;
    }
...
BOOL WINAPI WlxQueryTsLogonCredentials(PWLX_CLIENT_CREDENTIALS_INFO_V2_0 pInfo) {
    ...
    return WinStationQueryLogonCredentialsW( pInfo );
}
...
BOOL WsInAWorkgroup(VOID) {
    static BOOL fResult = FALSE, fCalculated = FALSE;
    if (!fCalculated) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (NT_SUCCESS(Status)) {
                fCalculated = TRUE;
                fResult = (PrimaryDomainInfo->Sid == NULL);
            }
    ...
    )
{
    ...
    if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE,
                       ...
                       &hKey ) == ERROR_SUCCESS )
    {
        cbData = sizeof(fWaitForNetwork);
        RegQueryValueEx( hKey,
                         TEXT("WaitForNetwork"),
                         NULL,
                         &dwType,
                         (LPBYTE)&fWaitForNetwork,
                         &cbData );
        RegCloseKey( hKey );
    }

    if ( !fWaitForNetwork && WsInAWorkgroup() ) {
        return;
    }
    ...
    if ( !IsMachinePolicyFgRefreshSync( pTerm ) ) {
        return;
    }
        ...
        WCHAR *Domain)
{
    ...
    if (pTerm != NULL) {
        ...
        if (rc == STATUS_SUCCESS) {
            ...
            if ( HasConfig ) {
                if ( ConfigData.User.WFProfilePath[0] != 0 ) {
                    wcscpy( TempString, ConfigData.User.WFProfilePath );
                    wcscat( TempString, TEXT("\\"));
                    wcscat( TempString, UserName );
                    wcscpy( pTSData->ProfilePath, TempString );
                    wcscpy( pTerm->MuGlobals.UserConfig.WFProfilePath, TempString );
                }
                if ( ConfigData.User.WFHomeDir[0] != 0 ) {
                    wcscpy( TempString, ConfigData.User.WFHomeDir );
                    wcscat( TempString, TEXT("\\") );
                    wcscat( TempString, UserName );
                    wcscpy( pTSData->HomeDir, TempString );
                    wcscpy( pTerm->MuGlobals.UserConfig.WFHomeDir, TempString );
                }
                if ( ConfigData.User.WFHomeDirDrive[0] != 0 ) {
                    wcscpy( TempString, ConfigData.User.WFHomeDirDrive );
                    wcscpy( pTSData->HomeDirDrive, TempString );
                    wcscpy( pTerm->MuGlobals.UserConfig.WFHomeDirDrive, TempString );
                }
            }
        }
