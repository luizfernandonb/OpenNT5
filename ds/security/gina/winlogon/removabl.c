...
BOOL
    RmvpCDRomsAllocated = FALSE,
    RmvpFloppiesAllocated = FALSE;
    ...
    )
{
    ...
    wcsncpy( ExtendedPath + 4, Path, 16 );
    ExtendedPath[20] = 0;
...
--*/
{
    ...
    if ( AllowEveryone )
    {
        ...
        SetMyAce( &Ace[AceCount], RmvpEveryoneSid, Access, 0 );
        AceCount++ ;
    }
    ...
    )
{
    ...
    for ( Path[0] = L'A' ; Path[0] < L'Z' ; Path[0]++ )
    {
        ...
        switch ( DriveType )
        {
            ...
            case DRIVE_CDROM:
                RmvpDoCdRom( arg_0, AllocatorId, Path );
                break;

            case DRIVE_REMOVABLE:
                RmvpDoRemovable( arg_0, AllocatorId, Path );
                break;
