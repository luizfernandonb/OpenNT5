    ...
    BOOL       fCleanup)
{
    ...
    if (hSvcMgr) {
        ...
        if (status != ERROR_MORE_DATA) {
             ...
             */
            NDDETRACE(("NDDEAgnt: EnumServicesStatus failed; GLE  = %d\n", status));
            CloseServiceHandle(hSvcMgr);
            retval = TRUE;
            goto exitIsServiceActive;
        }
    ...
    LPARAM lParam )    // additional information
{
    ...
    case WM_DDE_INITIATE:
        ...
        if (GlobalGetAtomName(LOWORD(lParam), szAppName, ARRAYSIZE(szAppName))) {
            ...
            if (_tcsicmp(szAppName, gszAppName) != 0) {
                 ...
                 */
                if (!GlobalGetAtomName(HIWORD(lParam), szAppName, ARRAYSIZE(szAppName))) {
    ...
    case WM_USERCHANGED:
        if (lParam == (LPARAM)gpCrtTerm->pWinStaWinlogon->hToken &&
             ...
             */
            dword_1075B7C = ImpersonateLoggedOnUser(gpCrtTerm->pWinStaWinlogon->hToken);
            if (!dword_1075B7C) {
                NDDETRACE(("NDDEAgntWndProc: WM_USERCHANGED: ImpersonateLoggedOnUser failed!"));
            }
        }
        break;
        ...
        if( CompareNddeModifyId( lpszShareName,
            ...
            if (dwOptions & NDDE_TRUST_SHARE_START) {
                ...
                bSuccess = CreateProcessAsUser(
                    gpCrtTerm->pWinStaWinlogon->hToken,
