...
RTL_CRITICAL_SECTION IniFileUserMappingLock;
    ...
    ActiveDesktops  Desktop)
{
    ...
    switch (Desktop)
    {
        ...
        default:
            ...
            if (fDesktopOpened && pWS->hdeskPrevious != NULL)
            {
                CloseDesktop(pWS->hdeskPrevious);
    ...
    )
{
    ...
    RtlEnterCriticalSection(&IniFileUserMappingLock);
    ...
    RtlLeaveCriticalSection(&IniFileUserMappingLock);
...
BOOL InitIniFileUserMappingSupport() {
    NTSTATUS status = RtlInitializeCriticalSection(&IniFileUserMappingLock);
    if (!NT_SUCCESS(status)) {
        DebugLog((DEB_ERROR, "InitIniFileUserMappingSupport failed to init its lock, error = 0x%08X\n", status));
        return FALSE;
    }
    return TRUE;
}
