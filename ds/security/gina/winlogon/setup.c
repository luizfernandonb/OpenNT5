...
BOOL
CheckForSetupRequiredShutdown(
    PDWORD pSetupShutdownRequired
    )
{
   DWORD SetupShutdownRequired = 0 ;
   BOOL fSuccess = FALSE;
   DWORD dwType, dwSize;
   HKEY hKeySetup;
   ...
   if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, KEYNAME_SETUP, 0,
       ...
       dwSize = sizeof(SetupShutdownRequired);
       if (RegQueryValueEx (hKeySetup, TEXT("SetupShutdownRequired"), NULL,
                        &dwType, (LPBYTE) &SetupShutdownRequired, &dwSize) == ERROR_SUCCESS)
       {
           if (dwType == REG_DWORD && SetupShutdownRequired <= 2) {
               *pSetupShutdownRequired = SetupShutdownRequired;
               fSuccess = TRUE;
               RegDeleteValue(hKeySetup, TEXT("SetupShutdownRequired"));
           }
       }
...

{
    ...
    USHORT LShift ;
    USHORT RShift ;
    BOOL fIsMiniNT = IsMiniNTMode();
    WCHAR WinStaNameBuffer[MAX_PATH];
    PCWSTR WinStaName;
    ...
    if(g_uSetupType != SETUPTYPE_UPGRADE && g_uSetupType != SETUPTYPE_NOREBOOT) {
    ...
    if (!fIsMiniNT) {
        ...
        wsprintf(WinStaNameBuffer, TEXT("%s\\Winlogon"), pTerm->pWinStaWinlogon->lpWinstaName);
        CallSetupSetDisplay(800, 600, 16);
    }
    ...
    WinStaName = WinStaNameBuffer;
    if (!fIsMiniNT) {
        WinStaName = TEXT("winsta0\\Default");
    }
...
BOOL HasEnoughDiskSpaceForHibernate(VOID)
{
    BOOL fRet = FALSE;
    ...
    if (GlobalMemoryStatusEx(&MemStatus)) {
        if (GetSystemDirectory(szSysDir, MAX_PATH)) {
            ...
            {
                if (FreeBytesAvailableToCaller.QuadPart > MemStatus.ullTotalPhys) {
                    fRet = TRUE;
                }
            }
        }
...
VOID EnableHibernation(VOID)
{
    SYSTEM_POWER_CAPABILITIES Capabilities;
    BOOLEAN fPrivilegeWasEnabled;
        ...
        )))
    {
        if (!Capabilities.HiberFilePresent) {
            BOOL fSysCanHibernate = (Capabilities.SystemS4 != FALSE);
            BOOL fDiskCanHibernate = HasEnoughDiskSpaceForHibernate();
            if (fSysCanHibernate && fDiskCanHibernate) {
                if (NT_SUCCESS(RtlAdjustPrivilege(
                    SE_CREATE_PAGEFILE_PRIVILEGE,
                    TRUE, FALSE, &fPrivilegeWasEnabled)))
                {
                    BOOLEAN yes = TRUE;
                    NtPowerInformation(SystemReserveHiberFile, &yes, sizeof(yes), NULL, 0);
                    RtlAdjustPrivilege(SE_CREATE_PAGEFILE_PRIVILEGE,
                        fPrivilegeWasEnabled, FALSE, &fPrivilegeWasEnabled);
                }
            }
        }
    }
    ...
    )
{
    ...
    if (RegOpenKeyEx(
        ...
        &hKey) == ERROR_SUCCESS)
    {
        if (RegQueryValueEx(hKey, TEXT("HibernationPreviouslyEnabled"),
            NULL, NULL, NULL, NULL) != ERROR_SUCCESS)
        {
            dwData = 1;
            RegSetValueEx(hKey, TEXT("HibernationPreviouslyEnabled"),
                0, REG_DWORD, (LPBYTE)&dwData, sizeof(dwData));
            EnableHibernation();
        }
        RegCloseKey(hKey);
    }
...
BOOL IsMiniNTMode() {
    HKEY hKey = NULL;
    static BOOL fIsMiniNT = FALSE, fCalculated = FALSE;

    if (!fCalculated) {
        DWORD err = RegOpenKey(
            HKEY_LOCAL_MACHINE,
            TEXT("SYSTEM\\CurrentControlSet\\Control\\MiniNT"),
            &hKey);
        fCalculated = TRUE;
...
{
    BOOL fRet = FALSE;
    if (dwMinWidth != 0 && dwMinHeight != 0 && dwMinBits != 0) {
        HMODULE hModule = LoadLibrary(TEXT("syssetup.dll"));
        if (hModule != NULL) {
            PSETUPSETDISPLAY pfnSetupSetDisplay =
                (PSETUPSETDISPLAY)GetProcAddress(hModule, "SetupSetDisplay");
            if (pfnSetupSetDisplay != NULL) {
                fRet = pfnSetupSetDisplay(NULL, NULL, NULL, NULL,
                    dwMinWidth, dwMinHeight, dwMinBits);
            }
            FreeLibrary(hModule);
        }
    }
