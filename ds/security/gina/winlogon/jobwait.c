    ...
    )
{
    ...
    if ( NT_SUCCESS( Status ) ) {
        hJobLockEvent = CreateEvent( NULL, FALSE, FALSE, NULL );
        if ( hJobLockEvent == NULL ) {
    ...
    )
{
    ...
    HANDLE ProcessToken, Token = NULL ;
    DWORD dwSize, dwType;
    NTSTATUS Status ;
    ...
    )
{
    ...
    while ( Job->Flags & WINLOGON_JOB_DELETING )
    {
        LeaveCriticalSection( &JobLock );
        WaitForSingleObject( hJobLockEvent, 500 );
        EnterCriticalSection( &JobLock );
    }
    ...
    Job->Flags |= WINLOGON_JOB_DELETED ;
