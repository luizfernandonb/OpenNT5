...
extern BOOL g_fReadyForShutdown;
    ...
    )
{
    ...
    if ( ShutdownInProgress ||
        ...
        DebugLog((DEB_TRACE, "StartSystemShutdown failed. A shutdown is already in progress!\n"));

    } else {

        if (!g_fReadyForShutdown) {
            Error = ERROR_NOT_READY;
            DebugLog((DEB_TRACE, "StartSystemShutdown failed. We are not ready to process this call!\n"));
        } else if (!bForceAppsClosed && pShutDownTerm && IsLocked(pShutDownTerm->WinlogonState)) {
            Error = ERROR_MACHINE_LOCKED;
            DebugLog((DEB_TRACE, "StartSystemShutdown failed. We are in a locked state!\n"));
        } else {
    ...
    )
{
    ...
    return StartSystemShutdown(
                         ...
                         Timeout,
                         bForceAppsClosed,
                         bRebootAfterShutdown ) == 0 ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
...

{
    ...
    DebugLog((DEB_TRACE, "Doing remote-initiated (%ws\\%ws) Reboot=%s, Force=%s\n",
        UserDomain, UserName,
        bRebootAfterShutdown ? "true" : "false",
        bForceAppsClosed ? "true" : "false"));
    ...
    ShutdownReason.cbSize = sizeof(ShutdownReason);
    ShutdownReason.uFlags = ExitWindowsFlags;
    ShutdownReason.dwReasonCode = ReasonCode;
    ShutdownReason.dwEventType = SR_EVENT_INITIATE_CLEAN;
    ShutdownReason.lpszComment = lpMessage->Buffer;
    RecordShutdownReason(&ShutdownReason);
...
--*/
{
    ...
    if ( ShutdownDelayInSeconds == 0 )
    {
        ...
        DebugLog((DEB_TRACE, "Doing immediate shutdown, LastGinaRet = %x, Flags = %x\n", GinaCode, *Flags));
