    ...
    )
{
    ...
    if (!g_fHelpAssistantSession && !IsActiveConsoleSession()) {
        ...
        if (WinStationQueryInformation(SERVERNAME_CURRENT,
                                       ...
                                       &Length))
        {
            RDFilter_ApplyRemoteFilter(pWS->UserProcessData.UserToken,
                                       ClientData.PerformanceFlags,
                                       TRUE,
                                       RDFILTER_SKIPTHEMESREFRESH);
        } else {
    ...
    )
{
    ...
    for (;;) {
       ...
       *lpTemp = TEXT('\0');
       ...
       if (lstrcmp(szValueName, TEXT("USERDNSDOMAIN")) != 0 || szValue[0] != TEXT('\\')) {
           //
    ...
    if (g_IsTerminalServer) {
        ...
        szValue[0] = 0;
        if (!WinStationNameFromLogonId(SERVERNAME_CURRENT, g_SessionId, szValue)) {
            if (IsActiveConsoleSession()) {
                lstrcpy(szValue, TEXT("Console"));
            }
        }
    ...
    if ( hinstShell32 )
    {
                ...
                SUCCEEDED(pfnSHGetFolderPath(NULL, CSIDL_APPDATA | CSIDL_FLAG_DONT_VERIFY, pWS->hToken, 0, szValue)) )
        {
            RegSetValueEx (hKey,
                           ...
                           (LPBYTE)szValue,
                           (lstrlen(szValue) + 1) * sizeof(TCHAR));
            SetEnvironmentVariable(APPDATA_VARAIBLE, szValue);
        }
