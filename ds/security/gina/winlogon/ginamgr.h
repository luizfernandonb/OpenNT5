...
typedef
BOOL (WINAPI * PWLX_GETCONSOLESWITCHCREDENTIALS)(
    PVOID, PVOID);

typedef
VOID (WINAPI * PWLX_RECONNECTNOTIFY)(
    PVOID);

typedef
VOID (WINAPI * PWLX_DISCONNECTNOTIFY)(
    PVOID);
...
typedef struct _GINASESSION {
    ...
    PWLX_GETCONSOLESWITCHCREDENTIALS pWlxGetConsoleSwitchCredentials;
    PWLX_RECONNECTNOTIFY      pWlxReconnectNotify;
    PWLX_DISCONNECTNOTIFY     pWlxDisconnectNotify;
...
BOOL LoadGinaDll(PTERMINAL pTerm, LPWSTR lpGinaName, PBOOL isBadGina);
