...
const TCHAR* scrnsave_str1() { return TEXT("scrnsave.scr"); }
    ...
    BOOL arg_C)
{
    ...
    SCREEN_SAVER_DATA ScreenSaverData;
    BOOL Success;
    WinstaState PriorState = 0; // Winsta_PreLoad
    DWORD BeginTime;
    DWORD EndTime;
    BOOL fGotInfo;
    DWORD ScreenSaveTimeout;
    BOOL bReconEventSignalledBefore = FALSE;
    ...
    if ( pTerm->bIgnoreScreenSaverRequest == TRUE )
    {
        DebugLog(( DEB_WARN, "Ignore screen saver request, skipping notification\n" ));
        return -1 ;
    }
    //
    ...
    fGotInfo = GetScreenSaverInfo( &ScreenSaverData );
    if (arg_C && !ScreenSaverData.fSecure)
    {
        ScreenSaverData.fSecure = ScreenSaverData.fAutoReturnToWelcome;
    }
    if (!fGotInfo || !ValidateScreenSaver(&ScreenSaverData))
    {
        //
        // Not a valid screen saver, so bail.
        //

        if (SystemParametersInfo(SPI_GETSCREENSAVETIMEOUT, 0, &ScreenSaveTimeout, FALSE)) {
            SystemParametersInfo(SPI_SETSCREENSAVETIMEOUT, ScreenSaveTimeout, NULL, FALSE);
        }
        DeleteScreenSaverInfo(&ScreenSaverData);
...

    {
        ...
        if (!GetUserObjectInformation( pTerm->pWinStaWinlogon->hdeskScreenSaver,
                                       UOI_NAME,
                                       Buffer,
                                       CurrentLength - iWinStaLen * sizeof(TCHAR),
                                       &LengthNeeded ) )
        {
    ...
    if (pTerm->MuGlobals.field_E70 && bReconEventSignalled)
    {
        ...
        bReconEventSignalledBefore = TRUE;
    ...
    if (ScreenSaverData.fSecure && pTerm->UserLoggedOn)
    {
        ...
        if (AllowFastUnlock &&
            ...
            pTerm->MuGlobals.field_E70 || bReconEventSignalledBefore)
        {
    ...
    if (pTerm->UserLoggedOn && pTerm->field_1484)
    {
        Result = WLX_SAS_ACTION_LOCK_WKSTA;
    }
            ...
            if ( wParam != WLX_SAS_TYPE_SCRNSVR_TIMEOUT )
            {
                ...
                else 
                {
                    ...
                    else 
                    {
                        ...
                        {
                            SendMessage( hDlg, WLX_WM_SCREENSAVER_DIED, 0, 0 );
                        }
                        else
                        {
                            PostMessage( hDlg, WLX_WM_SCREENSAVER_DIED, 0, 0 );
    ...
    )
{
    ...
    ScreenSaverData->fAutoReturnToWelcome = TRUE ;
    ...
    if ( OpenHKeyCurrentUser( pWS ))
    {
        ...
        if ( err == 0 )
        {
            ...
            if ( err == 0 )
            {
                if ( dwType == REG_SZ )
                {
                    ScreenSaverData->fAutoReturnToWelcome = atoi( Buffer ) == 0;
                }
            }
            ...
            {
                fIsActiveConsoleSession = IsActiveConsoleSession();
                if (!fIsActiveConsoleSession)
                {
                    dwSize = sizeof(TEXT("scrnsave.scr"));
                }
                ...
                if ( ScreenSaverData->ScreenSaverName )
                {
                    ...
                    {
                        lstrcpy(ScreenSaverData->ScreenSaverName, TEXT("scrnsave.scr"));
                        err = ERROR_SUCCESS;
                    }
    ...
    if (ScreenSaverData->ScreenSaverName == NULL
        || ScreenSaverData->ScreenSaverName[0] == 0
        || _wcsicmp(ScreenSaverData->ScreenSaverName, TEXT("(NONE)")) == 0)
    {
        ScreenSaverData->fEnabled = FALSE;
