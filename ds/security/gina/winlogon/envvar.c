...
BOOL CALLBACK MonitorResetWorkAreaEnumProc(HMONITOR hMonitor, HDC hdc, LPRECT lpRect, LPARAM lParam) {
    SystemParametersInfo(SPI_SETWORKAREA, 0, lpRect, SPIF_SENDCHANGE);
    return TRUE;
}
    ...
    )
{
    ...
    if (!bUserLoggedOn) {
        EnumDisplayMonitors(NULL, NULL, MonitorResetWorkAreaEnumProc, 0);
    }

    if (IsActiveConsoleSession() || !bUserLoggedOn) {
        UpdatePerUserSystemParameters(pWS->hToken, bUserLoggedOn != FALSE);
    }
